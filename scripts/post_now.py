...
# フック型をランダム選択（毎回違うパターンを強制）
hook_types = [
    ("B", "ランキング型", "【○○ランキングTOP3】の形式。🥇🥈🥉を使え。テーマは恋愛運・仕事運・金運・健康運・対人運からランダムに選べ"),
    ("C", "警告型", "「○○座さん、今週は気をつけて。」の形式。ネガティブ→でも対策ありの構造"),
    ("D", "限定数字型", "「12星座中、たった○つだけ。」の形式。2つか3つに限定"),
    ("A1", "予告×仕事型", "「来週、仕事で大きな話が」の形式。仕事・キャリアに関する予告"),
    ("A2", "予告×恋愛型", "「今週後半、恋が動く星座。」の形式。恋愛に関する予告"),
    ("A3", "予告×転機型", "「来月、人生が変わる星座。」の形式。人生の転機に関する予告"),
]
selected_hook = random.choice(hook_types)
hook_id, hook_name, hook_instruction = selected_hook
...
【今回使うフック型（これ以外は使うな）】
{hook_name}: {hook_instruction}

【絶対禁止】「来週、金運が動く」「来週、金星が動く」は使い過ぎたので禁止。新しい切り口で書け。
