...
# 7スロット: 07:07/09:07/10:07/12:07/15:07/19:07/21:42 JST
# 再現性テスト期間 (2026-04-06〜2026-04-30)
# 目的: 構造G固定レシピを5スロット×毎日投稿 → 7日で35件 → 成功率(v≥500)70%以上で「再現性確認」
# 構造F×2は比較対照群として継続
EXPERIMENT_TIME_SLOTS = [
    {"hour": 7,  "slot": "朝（6〜9時台）",     "structure": "G", "pattern_hint": "注意喚起+限定型①（今日・今週急に動く星座）"},
    {"hour": 9,  "slot": "朝（6〜9時台）",     "structure": "G", "pattern_hint": "注意喚起+限定型②（今週急変する・朝向け）"},
    ...
    {"hour": 21, "slot": "夜（19〜21時台）",   "structure": "F", "pattern_hint": "告白・暴露型②（実は〜という真実を伝えます）"},
]
...
8. 抽象的な共感型【閲覧数平均153・ER 8%】: 「今日も頑張りましょう」「自分を信じて」等、星座名も具体的天文イベントも含まない抽象的な感情共有投稿。※構造C（天文イベント×質問型）はスコア24で廃止済み。天文イベント系投稿は生成しない。

■ ⚠️ バリエーション必須ルール（同じパターンを2件以上連続で生成しない）:
- 10件生成する場合、構造G(注意喚起+限定型)は最大5件まで
- 構造F(告白型)を必ず2件入れる
- 構造E(カード選択型)を必ず1件入れる
...
4. トピックタグは末尾に1つだけ
5. パターン配分（10件必須）: 注意喚起+限定型5件（構造G・必須）、告白型2件（構造F・必須）、カード選択型1件（構造E・必須）、ランキング型1件（構造B）、その他1件（自由）※構造C（天文イベント×質問型）・構造D（予告型）・自由型は廃止
