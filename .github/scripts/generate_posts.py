...
- 10件生成する場合、構造G(注意喚起+限定型)は最大4件まで
- 構造H(よぞら.の声型)を必ず2件入れる（キャラクター確立・ストック型）
...
1. 10件生成（Threads専用）{'（うち1件はアフィリエイト投稿）' if should_generate_affiliate else ''}
   ※構成: G×4 + H×2 + I×1 + F×1 + E×1 + B×1（合計10件）
2. 各投稿は200-300文字（Threadsの最適値）
3. 全投稿で会話を誘導するCTAを含める（質問型中心）
4. トピックタグは末尾に1つだけ
5. パターン配分（10件必須）: 注意喚起+限定型4件（構造G・必須）、よぞら.の声型2件（構造H・必須）、問いかけ対話型1件（構造I・必須）、告白型1件（構造F・必須）、カード選択型1件（構造E・必須）、ランキング型1件（構造B）
...
- 使用可能な時間表現（厳守）:
  ...
  ・19〜21時台 → 「今夜」「今日」「今週」OK
- 「今夜○時までに」は19時以降スロットのみ使用可""" if slot_info["structure"] not in ("H", "I") else f"""- 構造{slot_info["structure"]}は時間に依存しないストック型コンテンツのため、
  特定の時刻・時間帯を示す表現（「今週」「今月」「今朝」「今夜」）は使わない
  （時限コンテンツになると翌日以降に価値がゼロになるため）"""}
